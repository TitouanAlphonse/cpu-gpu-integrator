...
void launch_integration(string integration_method, string integration_mode, Massive_body* mb,Test_particle* tp, int N_mb, int N_tp, double tau, int& N_step, double computation_time, bool def_N_step_in_cp_time, int N_substep, string user_forces_type, int nb_block, int nb_thread, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
    ...
    case 1:
        integration_CPU(mb, tp, N_mb, N_tp, tau, N_step, leapfrog::step_leapfrog_CPU, user_forces_type, leapfrog::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 2:
        integration_CPU(mb, tp, N_mb, N_tp, tau, N_step, computation_time, leapfrog::step_leapfrog_CPU, user_forces_type, leapfrog::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 3:
        integration_GPU(mb, tp, N_mb, N_tp, tau, N_step, nb_block, nb_thread, leapfrog::step_leapfrog_GPU, user_forces_type, leapfrog::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 4:
        integration_GPU(mb, tp, N_mb, N_tp, tau, N_step, computation_time, nb_block, nb_thread, leapfrog::step_leapfrog_GPU, user_forces_type, leapfrog::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 5:
        integration_GPU_multi_step(mb, tp, N_mb, N_tp, tau, N_step, N_substep, nb_block, nb_thread, leapfrog::substep_leapfrog_mb_GPU_multi_step, leapfrog::step_leapfrog_tp_GPU_multi_step, user_forces_type, leapfrog::pos_vel_sub_multi_step, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 7:
        integration_CPU(mb, tp, N_mb, N_tp, tau, N_step, MVS::step_MVS_CPU, user_forces_type, MVS::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 8:
        integration_CPU(mb, tp, N_mb, N_tp, tau, N_step, computation_time, MVS::step_MVS_CPU, user_forces_type, leapfrog::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 9:
        integration_GPU(mb, tp, N_mb, N_tp, tau, N_step, nb_block, nb_thread, MVS::step_MVS_GPU, user_forces_type, MVS::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
        break;
    case 10:
        integration_GPU(mb, tp, N_mb, N_tp, tau, N_step, computation_time, nb_block, nb_thread, MVS::step_MVS_GPU, user_forces_type, MVS::pos_vel_sub, file_pos, file_orb_param, freq_w, def_freq_w_in_steps);
    default:
    case 11:
...
__host__ void integration_CPU(Massive_body* mb, Test_particle* tp, int N_mb, int N_tp, double tau, int N_step, step_function_CPU step_update, string user_forces_type, pos_vel_sub_func pos_vel_sub, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
    ...
    User_forces user_forces(user_forces_type, enable_user_forces);
    ...
    for (int step=1; step<=N_step; step++) {
        ...
        if (enable_user_forces) {
            user_forces.apply(mb, N_mb, step*tau, tau, pos_vel_sub, M_tot);
        }
...
__host__ void integration_CPU(Massive_body* mb, Test_particle* tp, int N_mb, int N_tp, double tau, int& N_step, double computation_time, step_function_CPU step_update, string user_forces_type, pos_vel_sub_func pos_vel_sub, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
    ...
    while (chrono::duration_cast<chrono::milliseconds>(timer - start).count() < computation_time*60000) {
        ...
        step_update(mb, tp, N_mb, N_tp, tau, aux_mb);
        if (enable_user_forces) {
            user_forces.apply(mb, N_mb, N_step*tau, tau, pos_vel_sub, M_tot);
        }
...
__host__ void integration_GPU(Massive_body* mb, Test_particle* tp, int N_mb, int N_tp, double tau, int N_step, int nb_block, int nb_thread, step_function_GPU step_update, string user_forces_type, pos_vel_sub_func pos_vel_sub, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
...
__host__ void integration_GPU(Massive_body* mb, Test_particle* tp, int N_mb, int N_tp, double tau, int& N_step, double computation_time, int nb_block, int nb_thread, step_function_GPU step_update, string user_forces_type, pos_vel_sub_func pos_vel_sub, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
...
__host__ void integration_GPU_multi_step(Massive_body* mb, Test_particle* tp, int N_mb, int N_tp, double tau, int N_step, int N_substep, int nb_block, int nb_thread,
                                         substep_function_mb_GPU_multi_step substep_mb, step_function_tp_GPU_multi_step step_tp, string user_forces_type,
                                         pos_vel_sub_func_multi_step pos_vel_sub, ofstream& file_pos, ofstream& file_orb_param, double freq_w, bool def_freq_w_in_steps) {
    ...
    for (int step=0; step<N_step/N_substep; step++) {
        ...
        for (int substep=1; substep<=N_substep; substep++) {
            ...
            if (enable_user_forces) {
                user_forces.apply_multi_step(mb_qv_multi_step, mb_mR, N_mb, substep, (step*N_substep+substep)*tau, tau, pos_vel_sub, M_tot);
            }
